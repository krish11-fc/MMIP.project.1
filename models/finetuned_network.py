"""
models/finetuned_network.py — T-stage network using FinetunedDiffusionStage.
"""
import torch
...
class FinetunedInertialTNRDNetwork(nn.Module):
    """
    T-stage Inertial TNRD with learnable PDE scalars (K, γ, τ, ν, σ).

    Identical architecture to InertialTNRDNetwork but each stage uses
    FinetunedDiffusionStage instead of InertialDiffusionStage.
    Filters remain frozen; only the 5 scalars + RBF weights are learned.
    """
    ...
    def __init__(self, num_stages=10, num_filters=48, filter_size=7,
        ...
        # Fixed filter bank (same across all stages, like original)
        ...
        for t in range(num_stages):
            self.stages.append(FinetunedDiffusionStage(
